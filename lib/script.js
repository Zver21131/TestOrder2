...
function sendSms(numberOrder) {
    if (!confirm(`Вы уверены, что хотите отправить СМС по заказу ${numberOrder}?`)) {
        return;
    }
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success) {
                alert(res.message || "СМС отправлено успешно");
                location.reload();
            } else {
        ...
        error: function (xhr, status, error) {
            ...
            alert("Произошла ошибка связи с сервером: " + error);
        }
    });
}
